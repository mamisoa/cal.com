/**
 * TeamService - AGPL-3.0 Licensed
 *
 * Service layer for team operations.
 *
 * @module @calcom/features/teams/services/teamService
 * @license AGPL-3.0
 */

import { randomBytes } from "node:crypto";

import { deleteWorkfowRemindersOfRemovedMember } from "@calcom/features/teams/lib/deleteWorkflowRemindersOfRemovedMember";
...
import { getTeamBillingServiceFactory } from "@calcom/lib/stubs/billing";
...
export class TeamService {
  ...
  static async delete({ id }: { id: number }) {
    ...
    // In self-hosted mode, this is a no-op stub.
  ...
  static async removeMembers({
    ...
    // In self-hosted mode, billing updates are no-ops
    const teamBillingServiceFactory = getTeamBillingServiceFactory();
  ...
  static async publish(teamId: number) {
    // In self-hosted mode, publishing returns null URL (no checkout needed)
    const teamBillingServiceFactory = getTeamBillingServiceFactory();
