/**
 * WorkflowsPage - AGPL-3.0 Licensed
 *
 * Main workflows list page without license restrictions.
 */
"use client";

// Import WorkflowListType from EE for compatibility (the type itself is not EE-restricted)
...
// Using EE components temporarily - these can be copied to AGPL module later
...
type PageProps = {
  filteredList?: {
    filtered: WorkflowListType[];
    totalCount: number;
  };
};

/**
 * AGPL WorkflowsPage - No license check required
 */
function WorkflowsPage({ filteredList }: PageProps) {
  ...
  return (
    <Shell withoutMain>
      {/* No LicenseRequired wrapper - AGPL version */}
      <ShellMain
        heading={t("workflows")}
...
// Filter component kept for future use
const _Filter = (props: {
  profiles: {
